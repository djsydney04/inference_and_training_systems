...
function matmulDiagram({ config: c, step, frame: f }: MatmulState) {
  ...
  return `<div class="matrix-diagram" aria-label="Matrix multiplication: global operands, shared memory and output">
    ...
    <div class="matrix-shared"><p class="numerical-label">Shared memory <span>${f.phase === "start" ? "Waiting for the first load" : f.phase === "load" ? "Loaded; wait for the producer barrier" : "Producer barrier reached; operands may be read"}</span></p><div class="matrix-pair">${matrix("Shared A",shared(f.sharedA),(r,col)=>sharedClass(r,col,"A"))}${matrix("Shared B",shared(f.sharedB),(r,col)=>sharedClass(r,col,"B"))}</div></div>
