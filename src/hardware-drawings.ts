...
export function prepareHardwareDrawings() {
  for(const explorer of explorers) {
    ...
    if(!host)continue;
    host.outerHTML=`<figure id="${explorer.id}" class="hardware-drawing wide-figure" data-hardware-drawing="${explorer.id}" data-lesson="${esc(explorer.title)}"><figcaption><span>Hardware reference drawing</span><strong>${esc(explorer.title)}</strong><p>Select a named part to inspect it. Choose a view to look inside.</p></figcaption><div class="hd-tabs" role="group" aria-label="Hardware view">${explorer.ids.map((id,i)=>`<button type="button" data-hd-view="${id}" aria-pressed="${i===0}">${esc(views.find(v=>v.id===id)!.title)}</button>`).join("")}</div><div class="hd-canvas" tabindex="0" aria-label="Hardware drawing; scroll horizontally for all components"></div><div class="hd-inspector" aria-live="polite"></div><p class="hd-boundary"></p><a class="hd-source lesson-source" target="_blank" rel="noreferrer"></a></figure>`;
  }
}
