import { sketchBox } from "./inference-sketch";
...
function routingDiagram(routes: ExpertRoute[], counts: number[], selected: number, outputs: Matrix) {
  ...
  return `<svg viewBox="0 0 830 374" role="img" aria-label="Four token rows route to three experts and combine back to their original token positions. Highlighted token ${selected} uses ${routes.filter(r => r.token === selected).map(r => `expert ${r.expert} with gate ${n(r.weight)}`).join(" and ")}. Combined output ${vector(outputs[selected])}."><g class="moe-map-heading"><text x="24" y="25">Token rows X</text><text x="345" y="25">Expert groups</text><text x="654" y="25">Combined Y</text></g>${paths}${ty.map((y, t) => `<g class="${t === selected ? "moe-node-selected" : "moe-node"}">${sketchBox(12, y - 27, 158, 54, 'class="inference-sketch-box"')}<text x="24" y="${y - 5}">t${t} · ${vector(inputs[t])}</text><text class="moe-map-small" x="24" y="${y + 15}">input position ${t}</text>${sketchBox(640, y - 27, 180, 54, 'class="inference-sketch-box"')}<text x="652" y="${y - 5}">y${t}</text><text class="moe-map-small" x="652" y="${y + 15}">${vector(outputs[t])}</text></g>`).join("")}${ey.map((y, e) => `<g class="moe-node">${sketchBox(330, y - 31, 150, 62, 'class="inference-sketch-box"')}<text x="346" y="${y - 7}">Expert ${e} · W${e}</text><text class="moe-map-small" x="346" y="${y + 15}">${counts[e]} assigned rows</text></g>`).join("")}<text class="moe-map-small" x="278" y="363">Pack by expert → run xW → weight and scatter-add</text></svg>`;
}
