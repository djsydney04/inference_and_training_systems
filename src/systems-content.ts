...
<p>Padding fills unused slots when examples have different lengths. Packing places several shorter examples into one row. A worker is a participating process; a microbatch is one local forward/backward portion accumulated before a weight update.</p>
...
export const servingChapter = chapter(
  ...
  `
<section class="lesson" id="load-test" data-lesson="Latency, throughput and batching"><header><span>Serving experiments</span><h3>Define the traffic before choosing the batch</h3></header>
<p>One person cares how quickly their answer appears. The service operator also cares how many people can receive answers at once. These are different measurements. A batch lets several requests share a model execution; it does not let one request skip the dependency on its previous generated token.</p>
<div class="derivation"><h4>One user's speed and the service's throughput</h4><p>Suppose an illustrative decode step takes 20 ms and emits one token for each of four active requests. Each user receives <code>1 / 0.020 = 50 output tokens/s</code> during this steady decode interval. The service emits <code>4 / 0.020 = 200 output tokens/s</code> in total. The same number describes four people's combined output, so it cannot be advertised as one person's generation speed.</p><p>This is arithmetic with an assumed step time, not a hardware result. A larger batch can take longer per step. Measure its effect on each request as well as the combined output.</p></div>
<p><strong>Latency</strong> is elapsed time, reported in milliseconds or seconds. <strong>Throughput</strong> is completed work divided by the observation interval: requests/s or output tokens/s. Report input-token throughput separately. <strong>Concurrency</strong> counts requests active at once; <strong>arrival rate</strong> counts newly offered requests per second. Neither is a cache size.</p>
<div class="service-timeline"><span>Request start</span><b>→</b><div>Admission + queue</div><b>→</b><div>Prefill</div><b>→</b><strong>First token</strong><b>→</b><div>Decode / stream</div><b>→</b><span>Completion</span></div>
<p>Time to first token (<strong>TTFT</strong>) runs from the client's request start to its first received token. Inter-token latency (<strong>ITL</strong>) measures gaps during streaming. Time per output token (<strong>TPOT</strong>) summarizes the remaining generation time. For N output tokens, <code>TPOT = (total latency − TTFT) / (N − 1)</code> when N&gt;1, following the pinned benchmark below. One output token provides no subsequent-token interval.</p>
${reading("https://github.com/vllm-project/vllm/blob/v0.12.0/vllm/benchmarks/serve.py", "vLLM 0.12.0 source: throughput and per-request timing calculations")}
<p>The formula above follows that benchmark’s terminal-latency convention. When the protocol ends after the last token, a token-receipt TPOT uses the last-token timestamp instead. The <a href="#streaming-metrics">delivery ledger</a> keeps those boundaries separate and works through failures and bundled output.</p>
${exercise("A request returns six tokens: the first at 100 ms and the last at 200 ms. What are TTFT and TPOT?", "Using the last-token receipt boundary (and assuming completion is also at 200 ms for terminal TPOT), TTFT is 100 ms. Five intervals remain after the first token, so TPOT is (200−100)/5 = 20 ms/token. Those intervals could be 20,20,20,20,20 ms or 5,5,5,5,80 ms. Equal averages can hide a noticeable stall; inspect the individual gaps too.")}
<details class="deep-dive"><summary>Work through padding, continuous batching and token budgets</summary>
<p>A padded batch uses a rectangular tensor. Prompts of lengths 4, 2 and 1 need 12 slots when padded to length 4, but contain seven real positions: <code>7/12 ≈ 58%</code> of the slots are useful. Masks preserve the intended attention and target rules; skipping computation on padding requires kernel or runtime support. This slot fraction is not a measured GPU utilization percentage.</p>
<p><strong>Continuous batching</strong> changes batch membership between model iterations. If request A finishes before request B, A can leave while B continues. A waiting request C can enter once its prompt processing and cache allocation are scheduled. A fixed batch that waits for every request to finish cannot reuse the empty place this way. See the <a href="#serving-budgets">prefill and decode lesson</a> and <a href="#paged-kv">cache block diagram</a> for the state behind this schedule.</p>
${reading("https://www.usenix.org/conference/osdi22/presentation/yu", "Orca: the original iteration-level scheduling mechanism")}
<p><strong>Chunked prefill</strong> splits prompt processing across iterations. With a teaching budget of eight input positions per iteration, three ordinary one-token decodes leave five positions for a pending prompt chunk: <code>3 + 5 = 8</code>. This counts newly processed positions. It does not count all cached positions that attention reads, and it does not predict milliseconds. Speculative decoding uses a different per-request token count.</p>
<p>Keep two limits separate: the <strong>work budget</strong> controls positions scheduled now; the <strong>memory budget</strong> controls retained state. Suppose 1 MiB is available exclusively for KV payload and this example model needs 2 KiB per retained position. Capacity is <code>1,024/2 = 512 positions</code>. Two requests retaining 200 positions each already occupy 400 of those positions, even if their next step processes only two new tokens. Real admission must also account for allocation overhead, growing outputs and its reservation or preemption policy.</p>
${reading("https://docs.vllm.ai/en/v0.12.0/configuration/optimization/", "vLLM 0.12.0: chunked prefill, scheduling budgets and KV pressure")}
</details>
<h4>Then choose the workload</h4>
...
<p>State whether the measured interval includes queueing, network transit and tokenization. Streaming can bundle several tokens into one network event; a chunk-arrival gap is not automatically a token-generation gap. Publish distributions together with errors, timeouts and the population used for each percentile. Failed requests cannot silently disappear from a claim about service quality.</p>
${reading("https://docs.vllm.ai/en/v0.12.0/cli/bench/serve/", "vLLM 0.12.0: reproducible workload and measurement options")}
</section>
...
const project = (
  ...
  `<article class="engineering-project" id="${id}" data-lesson="${title}"><header><span>${focus}</span><h3>${title}</h3></header><p>${task}</p><dl><div><dt>Deliver</dt><dd>${evidence}</dd></div><div><dt>Review question</dt><dd>${review}</dd></div></dl></article>`;
export const projectsChapter = chapter(
  ...
  `
<div class="project-intro"><p>These projects grow from laptop-scale tests to real accelerator experiments. Passing a checklist is not equivalent to being ready for a particular employer; the work should stand up to a technical review. Do not substitute an illustrative browser diagram for a hardware measurement.</p><p>The manual includes TensorFlow and PyTorch references. For work in training and serving stacks, also become comfortable reading CUDA, compiler IR, distributed runtime code and Linux tooling. The transferable skill is reasoning below the framework abstraction.</p></div>
...
<section class="lesson" id="engineering-report" data-lesson="Write an engineering report"><header><span>How to review your work</span><h3>A short engineering report</h3></header><ol class="report-outline"><li><strong>Question.</strong> What hypothesis are you testing?</li><li><strong>Contract.</strong> Which outputs, data and quality constraints must remain unchanged?</li><li><strong>Method.</strong> Can another engineer reproduce the environment and workload?</li><li><strong>Evidence.</strong> Show correctness tests, traces, distributions and counterexamples.</li><li><strong>Conclusion.</strong> Explain the bottleneck, the improvement, the cost, and what remains unknown.</li></ol></section>`,
);
