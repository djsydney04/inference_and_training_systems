...
import type { ChipView } from "./chip-anatomy-data";
...
function diagram(view: ChipView) {
  ...
  const nodes = view.parts.map(p => `<g class="chip-node chip-${p.kind}" data-chip-part="${p.id}" role="button" tabindex="0" aria-pressed="false" aria-label="Inspect ${escape(p.label)}"><title>${escape(p.label)}: ${escape(p.detail)}</title><rect class="chip-node-body" x="${p.x}" y="${p.y}" width="${width}" height="${height}"/><text class="chip-node-label" x="${p.x + 14}" y="${p.y + 26}">${escape(p.label)}</text><text class="chip-node-detail" x="${p.x + 14}" y="${p.y + 46}">${escape(p.detail)}</text>${p.open ? `<text class="chip-open-mark" x="${p.x + width - 18}" y="${p.y + 76}" aria-hidden="true">↗</text>` : ""}</g>`).join("");
  return `<svg viewBox="0 0 988 ${bottom}" class="chip-svg" aria-labelledby="${view.id}-title ${view.id}-description"><title id="${view.id}-title">${escape(view.title)}</title><desc id="${view.id}-description">Select a named component or use the component menu. Arrows show directed relationships; dashed arrows show control. Sizes do not encode resource counts.</desc><defs><marker id="${arrow}" markerWidth="7" markerHeight="7" refX="6" refY="3.5" orient="auto"><path d="M0 0L7 3.5L0 7" fill="context-stroke"/></marker></defs>${edges}${nodes}</svg>`;
}
...
function viewMarkup(view: ChipView, first: boolean) {
  return `<div class="chip-view" data-chip-panel="${view.id}"${first ? "" : " hidden"}>
    ...
    <div class="chip-legend"><span>Solid: data path</span><span>Dashed: control dependency</span></div>
    ...
    <div class="chip-support">    <details class="chip-options"><summary>Find / zoom</summary>    <div class="chip-controls"><label>Component<select data-chip-picker aria-label="Component in ${escape(view.label)}">${view.parts.map(p => `<option value="${p.id}">${escape(p.label)}</option>`).join("")}</select></label><label>Diagram size<select data-chip-zoom aria-label="Diagram size in ${escape(view.label)}"><option value="1">100%</option><option value="1.3">130%</option><option value="1.6">160%</option></select></label></div></details>
    <details class="chip-route-details"><summary>Trace a route</summary><div class="chip-trace"><label>Follow a route<select data-chip-route>${view.routes.map((r, i) => `<option value="${i}">${escape(r.label)}</option>`).join("")}</select></label><div class="chip-trace-buttons"><button type="button" data-chip-prev aria-label="Previous route step">Previous</button><button type="button" data-chip-next>Start route</button><button type="button" data-chip-reset>Explore freely</button></div><p class="chip-trace-status" data-chip-status aria-live="polite">Select a component or start a route.</p><p class="chip-route-note">${escape(view.routes[0].note)}</p></div></details>
    <details class="deep-dive chip-reference"><summary>Notes &amp; reference</summary><p class="chip-boundary">${escape(view.boundary)}</p><div class="chip-reference-list">${view.parts.map(p => `<article><h4 id="${view.id}-${p.id}-reference">${escape(p.label)}</h4><p>${escape(p.body)}</p><p><strong>Watch for:</strong> ${escape(p.watch)}</p><p><strong>Example:</strong> ${escape(p.example)}</p></article>`).join("")}</div></details></div>
  </div>`;
}
...
export function prepareChipAnatomy() {
  chipExplorers.forEach(explorer => {
    ...
    host.insertAdjacentHTML("afterend", `<section class="lesson chip-anatomy" id="${explorer.id}" data-lesson="${escape(explorer.title)}"><header><span>Chip anatomy</span><h3>${escape(explorer.title)}</h3></header><p>${escape(explorer.intro)}</p><figure class="chip-explorer" data-chip-explorer="${explorer.id}"><figcaption><span>Interactive component schematic</span><strong>${escape(explorer.title)}</strong></figcaption>${explorer.views.length > 1 ? `<div class="chip-view-tabs" role="group" aria-label="${escape(explorer.title)} views">${explorer.views.map((view, i) => `<button type="button" data-chip-view="${view.id}" aria-pressed="${i === 0}">${escape(view.label)}</button>`).join("")}</div>` : ""}${explorer.views.map((view, i) => viewMarkup(view, i === 0)).join("")}</figure><div class="chip-sources">${[...sources].map(([url, label]) => `<a class="lesson-source" href="${url}" target="_blank" rel="noreferrer">${escape(label)}</a>`).join("")}</div></section>`);
  });
}
