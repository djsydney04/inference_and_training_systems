  ...
  if (host && !host.dataset.initialized) {
    ...
    const render = () => {
      ...
      host.querySelector("[data-tree-diagram]")!.innerHTML = `<svg viewBox="0 0 470 326" role="img" aria-label="Prefix P branches into A and B. A branches into C and D; B into E; C into F; E into G. Selected path is ${path.map(i => names[i]).join(", ")}."><g>${parents.map((parent, i) => {
        ...
        return `<path d="M${px},${py + 20} L${x},${y - 25}" class="${path.includes(i) ? "tree-edge-selected" : "tree-edge"}"/>`;
      }).join("")}</g><rect x="167" y="10" width="182" height="42" class="tree-prefix"/><text x="258" y="37" text-anchor="middle">Prefix P · positions 0–3</text>${points.map(([x, y], i) => `<g><rect x="${x - 48}" y="${y - 25}" width="96" height="48" class="${i === selected ? "tree-query" : path.includes(i) ? "tree-ancestor" : "tree-other"}"/><text x="${x}" y="${y - 5}" text-anchor="middle" class="${i === selected ? "tree-query-label" : ""}">${names[i]}</text><text x="${x}" y="${y + 13}" text-anchor="middle" class="tree-coordinate ${i === selected ? "tree-query-label" : ""}">pos ${layout.positions[i]} · slot ${layout.storageSlots[i]}</text></g>`).join("")}</svg>`;
