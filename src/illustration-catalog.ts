...
export const studies = {
  ...
  gradient: { title: "From prediction to weight update", alt: "A batch passes through three model layers to a loss. Blue paths carry sensitivities backward to gradient tensors. An optimizer updates the weights and returns them to the model for the next batch." },
...
export const lessonStudies: { target: string; study: Study; title: string; note: string }[] = [
  ...
  { target: "first-weight-update", study: "gradient", title: "From one batch to updated weights", note: "A forward pass produces predictions and a loss. Backward computes parameter gradients; the optimizer applies an update before the next batch. A step need not lower the loss on every example. Tensor cells and layer counts are illustrative." },
  { target: "autodiff", study: "gradient", title: "Trace sensitivity back to the weights", note: "Local derivatives connect an output loss to the parameters that produced it. Forward values must be saved or recomputed for backward. The optimizer then uses the gradients to update the weights; the drawing's tensor shapes are illustrative." },
  ...
  { target: "optimizer-state", study: "gradient", title: "Gradients become a parameter update", note: "The optimizer combines gradients with the current parameters and any stored history. Momentum and adaptive methods carry state from earlier steps, so the same current gradient can produce a different update. Tensor shapes are illustrative." },
