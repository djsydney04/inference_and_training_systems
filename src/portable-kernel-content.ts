...
<p>Our common operation is <code>C[M,N] = A[M,K] × B[K,N]</code>. An output tile owns a rectangle of C and accumulates products across K. Each update reads an A tile and a B tile; it must preserve the same output ownership, valid bounds and numerical contract when the backend changes. The workbench counts storage for BF16 operands with FP32 partial sums. Its teaching tile sizes are choices, not hardware limits or benchmark winners. The executable scalar CUDA/HIP companion below instead uses FP32 inputs and FP32 accumulation; it checks indexing and launch coverage, and does not simulate BF16 rounding or matrix instructions.</p>
${portableKernelLab}
