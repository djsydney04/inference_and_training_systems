...
export function initializeFrameworkServingLabs() {
  ...
  if (cacheLab && !cacheLab.dataset.initialized) {
    ...
    const render = () => {
      ...
      find("[data-serving-cache-diagram]").innerHTML = `<div class="framework-cache-rows"><div class="framework-cache-row"><strong>Cached evaluated prefix</strong><div>${groups(baseline, false)}</div></div><div class="framework-cache-row"><strong>Incoming request</strong><div>${groups(incoming, true)}</div></div></div>`;
      find("[data-serving-cache-result]").textContent = `${result.commonTokens} of 10 leading token/position pairs match. ${result.reusableBlocks} complete ${result.reusableBlocks === 1 ? "block" : "blocks"} (${result.reusableTokens} tokens) ${result.reusableBlocks === 1 ? "is" : "are"} eligible for reuse.${result.mismatches.length ? ` Context mismatch: ${result.mismatches.join(", ")}.` : " Context identity matches."}`;
