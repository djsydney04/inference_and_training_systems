...
| `systems-mental-models`, `systems-scale-ladder`, `training-lifecycle` | Removed undefined memory acronyms from the opening; distinguished fitting training targets from evaluating held-out tokens; expanded SFT/RL vocabulary and avoided a mandatory SFT→DPO→RL sequence. Reviewed the newly merged six scale illustrations: scalar/tensor links now open their definitions, and a physical rack is no longer equated with a network scale-up domain. |
