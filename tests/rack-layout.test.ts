...
test("rack drawings share the reference component inventory", () => {
...
test("rack front view keeps the central switch group between unequal compute groups", () => {
  const groups: [string, number][] = [];
  for (const kind of rackSlotKinds) {
    if (groups.at(-1)?.[0] === kind) groups.at(-1)![1]++;
    else groups.push([kind, 1]);
  }
  assert.deepEqual(groups, [["power",4], ["compute",10], ["switch",9], ["compute",8], ["power",4]]);
});
